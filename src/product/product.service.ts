...
import { DataSource, IsNull, LessThan, LessThanOrEqual, MoreThan, Or, Repository } from 'typeorm';
...
export class ProductService {
  ...
  async findAll() { //Default sort by QUANTITY DESC
    return await this.dataSource.manager.createQueryBuilder(Product, 'product')
    .leftJoin(
      (subquery) => {
        return subquery
  ...
  async findActiveProducts(){
    return await this.dataSource.manager.getRepository(Product).find({
      where: [
        {SellStartDate: LessThanOrEqual(new Date())},
        {SellEndDate: Or(MoreThan(new Date()), IsNull())}
      ]
    })
  }
